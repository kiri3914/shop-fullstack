...
#         self.fields['username'].label = 'Логин'
#         self.fields['password'].label = 'Пароль'
#         self.fields['confirm_password'].label = 'Потдвердить пароль'
#         self.fields['phone'].label = 'Номер'
#         self.fields['address'].label = 'Адрес'
...
#         domain = email.split('.')[-1]
#         if domain in ['com', 'net']:
#             raise forms.ValidationError('Почта уже регистрирована')
#         if User.objects.filter(email.email).exists():
#             raise forms.ValidationError('Данная почта уже регас')
#
#     def clean_username(self):
#         username = self.cleaned_data['username']
#         if User.objects.filter(username=username).exists():
